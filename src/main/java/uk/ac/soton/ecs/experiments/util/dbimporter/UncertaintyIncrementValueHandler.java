...
public class UncertaintyIncrementValueHandler implements
		ConfigurationValueHandler {

	public ColumnName getColumnName() {
		return ColumnName.UNCERTAINTY_INCREMENT;
	}

	public String handle(File directory) {
		File simulationFile = new File(directory, "simulation.xml");

		Pattern pattern = Pattern.compile("uncertaintyIncrement.*value=.*\"",
				Pattern.MULTILINE);
		Matcher matcher;
		try {
