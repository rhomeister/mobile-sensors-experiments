...
public class LengthScaleValueHandler implements ConfigurationValueHandler {

	public ColumnName getColumnName() {
		return ColumnName.LENGTHSCALE;
	}

	public String handle(File directory) {
		File simulationFile = new File(directory, "simulation.xml");

		Pattern pattern = Pattern.compile("lengthScale.*value=.*\"",
				Pattern.MULTILINE);
		Matcher matcher;
		try {
