...
public class NonMyopicCoordinateResultsAnalyser {
	...
	public static void main(String[] args) throws Exception {
		...
		String query = "SELECT tau, gamma, experiment_set.id AS id, name, average_field_value, max_field_value, sensor_count "
				...
				+ "experiment_set.id=experiment_result.experiment_set_id AND experiment_set.experiment_group_id = 12";

		query = "SELECT tau, gamma, experiment_set.id AS id, name, AVG(average_field_value), AVG(max_field_value), STD(average_field_value), STD(max_field_value), sensor_count, uncertainty_increment, count(*) "
				+ "FROM experiment_set, experiment_result WHERE uncertainty_increment = 0.02 AND "
				+ "experiment_set.id=experiment_result.experiment_set_id AND experiment_set.experiment_group_id = 12 GROUP BY experiment_set.id "
		...
		while (result.next()) {
			...
			System.out.println(String.format(
					"%3d %3d %4d %10f %10f %10f %10f %10f %d", sensorCount, id,
					tau, uncertaintyIncrement, averageFieldValue,
					stdAverageFieldValue, maxFieldValue, stdMaxFieldValue,
					count));
		}
